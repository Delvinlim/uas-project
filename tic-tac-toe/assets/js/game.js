...
// Triggered when waiting someone to join, rotates the spinny symbol
degreeRotation = 0 
function spinObject(){
...
// Sandwiches text with the spinners
function sandwichWithSpinners(elementId, sandwichText){
	...
	waitSpinner1.setAttribute("class", "waitSpinner fa fa-spinner text-white")
	waitSpinner2.setAttribute("class", "waitSpinner fa fa-spinner text-white")
...
function createRematchButton(){	
	...
	rematchBtn.setAttribute("class", "actionBtn btn btn-lg btn-info")
	rematchBtn.setAttribute("onClick", "restartGame()")
	
	document.getElementById("btnState").appendChild(rematchBtn)
}

// Button that is made when opponent leaves to find a new game
function createFindGameButton(){
	var findGame = document.createElement("button")
	findGame.setAttribute("class", "actionBtn btn btn-lg btn-info")
	...
	document.getElementById("btnState").appendChild(findGame)
}

// Function that deals with reseting the game board when a new game started
function resetBoxes(){
	for (var i = 1; i < 10; i ++){
...
// Function returns text based on if it is your turn
function checkTurn(){
	var turnText;
	if (yourTurn){
		clearInterval(loop)
...
// Functions that deal with start of the game things
...
// Function to check if player disconnected
function playerDisconnected(text){
	// This clears any spinners if they are running
	clearInterval(loop)
		
	document.getElementById("disconnectState").innerHTML = text
...
// Runs when other trying to join with private room id but doesn't exist
...
// Function for game rematch
function restartGame(){
	document.getElementById("rematchButton").remove()
	document.getElementById("gameState").style.fontSize = "90%";
...
// if want use sweet alert put it right here letter
